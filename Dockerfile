...
# # install deepdetect
# RUN DEBIAN_FRONTEND="noninteractive" apt-get install -y build-essential libgoogle-glog-dev libgflags-dev \
...
# FROM jolibrain/deepdetect_cpu as base
# # RUN apt update
# RUN printf 'y\n1\n\1n' | apt install nodejs
# RUN apt install -y npm
...
CMD /opt/deepdetect/start-dede.sh -host 0.0.0.0; node -v
VOLUME ["/data"]
EXPOSE 8080